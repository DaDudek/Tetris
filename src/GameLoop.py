...
import pygame
from events.MainLoopEventHandler import handle_events, FALLING_EVENT, SPEED_UP_EVENT
...
def game_over_loop(points: int) -> None:
    ...
    while run:
        ...
        message_text_surf = game_message_font.render("GAME OVER, PRESS ANY BUTTON TO LEAVE", True,
                                                     get_black().get_color())
        player_text_surf = score_font.render(f"YOUR SCORE: {points}", True, get_black().get_color())
        window_surface.blit(player_text_surf, player_text_surf.get_rect(center=window_surface.get_rect().center))
...
player_score = main_game_loop(player_name)
game_over_loop(player_score)
